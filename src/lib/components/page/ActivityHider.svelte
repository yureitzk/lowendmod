<script lang="ts">
	...
	onMount(() => {
		const parent: HTMLElement | null = buttonElement.closest('div');
		if (parent) {
	...
	const handleClick = async () => {
		...
		try {
			if (isHidden) {
				...
				toggleActivities(username, true);
				toggleActivitiesComments(username, true);
			} else {
				hiddenUsers.add(user);
				toggleActivities(username, false);
				toggleActivitiesComments(username, false);
			}
		} catch (error) {
			console.error('Failed to update hidden status in store:', error);
		}
	};
</script>
